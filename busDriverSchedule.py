        ...
        for x in lisp[count]:
            ...
            for y in prefOff[count]:
                if y not in x:
                    ...
                    pLost+=4
            temp.append(dict(name=x,score=scor,lost=pLost,remove=0,reg=0))
            if(scor>maxi):
                maxi=scor
        ...
        for x in lisp[count]:
            ...
            for y in lisp[count+1]:
                ...
                if(pLost<32): #this if does the job of removeLow()
                    temp.append(dict(name=x['name']+y['name'],score=scor,lost=pLost,remove=x['remove']+y['remove'], reg=x['reg']+y['reg']))
...
def scoreGoods(lisp):
    ...
    for x in lisp:
        ...
        pLost+=from4+from3
        if(pLost<32):
            L.append(dict(name=nm, score=sc, lost=pLost, remove=(from4+from3), reg=from3G))
