...
def genDayByDay(days,off):
    l = []
    #this function creates a list of all shifts
    #for each day, 1-14, and removes shifts that violate
    #the day-off requirements
    #day off requirments != to preferences
    for st in off:
        temp = []
        for x in days:
            boo = True
...
def buildBridge(lisp):
    #this connects touching shifts (right side odd numbers remain unchanged)
    #works with different lengths
    #only keeps plans that don't require a driver
    #to work the night before an early shift
    count = 0;
    danielBoo=False
    if(len(lisp)%2!=0):
        danielBoo=True
    ...
    while(count < jon):
        ...
        for x in lisp[count]:
            ...
            for y in lisp[count+1]:
                boo = True
                for z in y['name'][:3]:       #the next morning
                    if z in x['name'][leng:]: #the night before
                        boo = False
                if(boo):
...
#removes all but the best n items in a list
#embedded inside another list
#will not change lists shorter than n
...
#new print functions might be usefull
...
def test5():
    ...
    a = removeLow(buildBridge(c),1000)#2->1
    b = removeLow(buildBridge(d),1000)#3->2
    c = removeLow(buildBridge(e[:3]+a+e[7:9]+b),1000)#8->4
    a = removeLow(buildBridge(c),1000)#4->2
    b = buildBridge(a) #2->1
    a = scoreGoods(b[0])
    return a
...
def test4():
    ...
    b = removeLow(buildBridge(a),1000)
    a = removeLow(buildBridge(b),1000)
    b = buildBridge(a)
    a = scoreGoods(b[0])
    return a
